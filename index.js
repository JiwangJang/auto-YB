...
function render() {
  ...
  `;
    ...
    order.forEach(({ worker, account, degree }) => {
        ...
        target.forEach((data) => {
            data.trip.forEach(({ isUp240, isUseCar, ignore }) => {
                if(ignore) return;
                if (isUp240) {
                    if (isUseCar)
                        up240AndUseCar.push({
                    ...
                    else up240.push({ date: data.date, isUp240, isUseCar });
                } else down240.push({ date: data.date, isUp240, isUseCar });
                totalTrip++;
            });
        });
...
document.querySelector("#excel-input").addEventListener("input", async (e) => {
    ...
    fileDatas.forEach((data) => {
        ...
        if (originIndex == -1) {
            ...
            originOrder.push({ worker: data.worker, account: "", degree: "" });
        } else {
    ...
    .addEventListener("click", (e) => {
        ...
        if (e.target.innerText === "수정") {
            ...
            order.forEach(({ worker, account, degree }) => {
                ...
                target.forEach((data) => {
                    data.trip.forEach(({ isUp240, isUseCar }) => {
                        if (isUp240) {
                            if (isUseCar)
                                up240AndUseCar.push({
                                    date: data.date,
                ...
                <div class="person-info-item" data-name=${worker}>
                    ...
                    <div class="col-bank"><input type="text" class="bank-input" value="${account.split(' ')[0] ?? ""}" /></div>
                    <div class="col-account" style="flex:2;"><input type="text" class="account-input" value="${account.split(' ')[1] ?? ""}" /></div>
